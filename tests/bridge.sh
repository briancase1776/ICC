...
#          payload bigger than one lane holds on one, take it off as
#          text, see the text is the alphabet and nothing else, put it on
#          the other, read it back whole, compare bytes, both directions;
#          an empty payload; text that is not the alphabet puts nothing
#          on the wire, and a wire with nothing on it prints nothing;
#          remove the pipes.
# @stdin nothing
