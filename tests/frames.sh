...
pDir=$("$pIccPipes/create" 6); osMade=$pDir
trap 'for pPipe in $osMade; do "$pIccPipes/remove" "$pPipe" 2>/dev/null || :; done
      cd /; rm -rf "$pWork"' EXIT
...
nStatus=0; timeout 1 "$pIccFrames/read" "$pDir" 1 >/dev/null 2>&1 || nStatus=$?
[ "$nStatus" -eq 124 ]
...
timeout 60 "$pIccFrames/write" "$pDir" 0 < big & pidWrite=$!
timeout 60 "$pIccFrames/read" "$pDir" 1 > bigout & pidRead=$!
wait $pidWrite; wait $pidRead; cmp big bigout
...
pDir2=$("$pIccPipes/create" 2); osMade="$osMade $pDir2"
...
timeout 60 "$pIccFrames/write" "$pDir2" 0 < in2 & pidWrite=$!
timeout 60 "$pIccFrames/read" "$pDir2" 1 > out2 & pidRead=$!
wait $pidWrite; wait $pidRead; cmp in2 out2
timeout 60 "$pIccFrames/write" "$pDir2" 1 < in2 & pidWrite=$!
timeout 60 "$pIccFrames/read" "$pDir2" 0 > out2 & pidRead=$!
wait $pidWrite; wait $pidRead; cmp in2 out2
